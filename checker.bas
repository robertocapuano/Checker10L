...
20 CE=CK((R+R0)/2,(C+C0)/2):if(Q=0 and R<>R0+2)or abs(R-R0)<>2 or abs(C0-C)<>2 or CK(R,C)<>0 or(CE and2)<>2 or Q<(CEand4) then C=C0:R=R0:goto10:else:CK((R+R0)/2,(C+C0)/2)=0:CK(R,C)=(CK(R0,C0)or(4*(-1*(R=7)))):CK(R0,C0)=0:P1=P1+1:vpokeB+479+P1,p(CE):beep
...
65 D=-(V<2)++(V>5)+(V>1)*(V<6)*K:T=-(U<2)++(U>5)+(U>1)*(U<6)*L:CE=CK(U+T,V+D): if (CEand1)=1 and Q>=(CEand4) and CK(U+2*T,V+2*D)=0 then :CK(U+2*T,V+2*D)=CK(U,V):CK(U,V)=0:CK(U+T,V+D)=0:I=7:J=7:K=1:AI=1:vpokeB-120+P2,p(1):P2=P2+1:beep
70 nextL,K,I,J:if P2=12 then locate0,0:?"CPU WINS": k$=INPUT$(1) : goto 1 else if AI=1 then 10
71 S=SGN(RND(1)-.5):forJ=0to7:U=(J+DU)mod8:forI=0to7:Q=CK(U,V)and4:V=(I+DV)mod8:forK=-1to1 step 2:forL=-1to(-(Q=4))step2: if (L=-1 and U<1) or (L=1andU>6) or (CK(U,V)and2)<>2 then75
73 D= -(V<1) + +(V>6) + (V>0) * (V<7)*K: T= -(U<1) + +(U>6) + (U>0) * (U<7)*L: if CK(U+L,V+D)=0 then CK(U+L,V+D)=(CK(U,V)or(4*(-1*((U+L)=0)))):CK(U,V)=0:K=11: AI = 1: J=7:I=7:L=1
